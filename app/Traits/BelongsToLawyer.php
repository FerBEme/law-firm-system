...
trait BelongsToLawyer {
    ...
    public function scopeForCurrentLawyer(Builder $query) {
        /** @var User $user */
        $user = Auth::user(); // Obtenemos el usuario actualmente autenticado        
        if ($user->isLawyer()) { // Si el usuario es un abogado, solo vemos registros donde él es el lawyer_id
            return $query->where('lawyer_id', $user->id);
        }
        if ($user->isSecretary()) { // Si el usuario es secretaria, solo vemos registros del abogado al que está asignada
